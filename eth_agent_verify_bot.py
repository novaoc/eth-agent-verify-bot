...
async def start_verify_cmd(interaction: discord.Interaction):
    ...
    ask_agent_prompt = (
        ...
        "identity standard: each agent is an ERC-721 NFT minted by an "
        "Identity Registry contract, and your tokenId IS your agentId.\n\n"
        "The Discord verifier bot checks against this specific registry:\n"
        f"  Contract: {default_reg}\n"
        f"  Chain:    Base mainnet (chainId {CHAIN_ID})\n\n"
        "Check whether you control an NFT minted by that contract:\n"
        "  1. Identify the wallet address you can sign transactions from.\n"
        f"  2. Call balanceOf(yourAddress) on {default_reg}. If it returns "
        "0, you hold no agent NFTs there.\n"
        "  3. If balanceOf > 0, you own at least one agent. For any "
        "tokenId you own, ownerOf(tokenId) will equal your address — "
        "that tokenId IS an agentId. Alternatively, the agent's metadata "
        "may set getAgentWallet(agentId) to a delegated signing wallet — "
        "if you control that address instead of the owner, you also pass "
        "the verifier's check.\n\n"
        "If YES (your wallet holds an ERC-721 token minted by that "
        "contract on Base, OR getAgentWallet of an existing agent "
        "delegates to a wallet you control):\n"
        "  Reply with ONLY this single line, no quotes, no other text:\n"
        f"    eip155:{CHAIN_ID}:{default_reg}:<agentId>\n"
        "  If you own multiple agents, ask the user which to verify with.\n\n"
        "If NO: reply with exactly NOT_REGISTERED. The user will run "
        "/register_help to register you on that contract."
    )
    verify_template = (
